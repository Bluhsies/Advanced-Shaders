...
//The main scene in this lab is outdated and before the tessellation shader was implemented.
//This is mainly used to test compute shaders and apply post processing to a texture in the "Compute" tab.
Lab8::Lab8(GLFWWindowImpl& win) : Layer(win)
{
	...
	floorShaderDesc.vertexSrcPath = "./assets/shaders/Lab8/Lab8GBVertFloor.glsl";
	floorShaderDesc.geometrySrcPath = "./assets/shaders/Lab8/floorPhongGeo.glsl";
	floorShaderDesc.fragmentSrcPath = "./assets/shaders/Lab8/Lab8GBFragFloor.glsl";
	...
	bbShaderDesc.vertexSrcPath = "./assets/shaders/Lab8/Lab8BBVert.glsl";
	bbShaderDesc.geometrySrcPath = "./assets/shaders/Lab8/bbPhongGeo.glsl";
	bbShaderDesc.fragmentSrcPath = "./assets/shaders/Lab8/Lab8BBFrag.glsl";
	...
	**************************/
	...
	//Here I have created two compute shaders which apply post processing (one inverts a texture and the other blurs it)
	//Compute Shader (invert)
	...
	std::shared_ptr<Texture> outImageTexture = std::make_shared<Texture>(textDesc);

	//I use this image for a bit of fun. I hope you don't mind!
	//Research in Societal Enhancement, n/a. Dr. Conor Fahy. Image available at: https://rise.our.dmu.ac.uk/about-rise/
	//Image accessed on: 21/01/2025.
	std::shared_ptr<Texture> inputImageTexture = std::make_shared<Texture>("./assets/textures/mario/conor.png");

	ShaderDescription computeDesc;
	computeDesc.type = ShaderType::compute;
	computeDesc.computeSrcPath = "./assets/shaders/Lab8/Lab8Compute.glsl";

	std::shared_ptr<Shader> computeShader;
	computeShader = std::make_shared<Shader>(computeDesc);

	std::shared_ptr<Material> computeMat;
	computeMat = std::make_shared<Material>(computeShader);
	computeMat->setValue("u_image", inputImageTexture);
	computeMat->setValue("u_imageSize", glm::vec2(512, 512));

	//I assign the number of work group and apply the memory barrier.
	ComputePass computePass;
	computePass.material = computeMat;
	computePass.workgroups = { 32, 32, 1 };
	computePass.barrier = MemoryBarrier::ShaderImageAccess;

	Image img;
	img.mipLevel = 0;
	img.layered = false;
	img.texture = outImageTexture;
	img.imageUnit = computePass.material->m_shader->m_imageBindingPoints["outputImg"];
	img.access = TextureAccess::WriteOnly;

	computePass.images.push_back(img);
	m_mainRenderer.addComputePass(computePass);

	// blur (compute)

	TextureDescription textDescb;
	textDescb.width = 512;
	textDescb.height = 512;
	textDescb.channels = 4;
	textDescb.isHDR = false;

	std::shared_ptr<Texture> outImageTextureb = std::make_shared<Texture>(textDescb);

	ShaderDescription computeDescb;
	computeDescb.type = ShaderType::compute;
	computeDescb.computeSrcPath = "./assets/shaders/Lab8/Lab8ComputeBlur.glsl";

	std::shared_ptr<Shader> computeShaderb;
	computeShaderb = std::make_shared<Shader>(computeDescb);

	std::shared_ptr<Material> computeMatb;
	computeMatb = std::make_shared<Material>(computeShaderb);
	computeMatb->setValue("u_image", inputImageTexture);
	computeMatb->setValue("u_imageSize", glm::vec2(512, 512));

	ComputePass computePassb;
	computePassb.material = computeMatb;
	computePassb.workgroups = { 32, 32, 1 };
	computePassb.barrier = MemoryBarrier::ShaderImageAccess;

	Image imgb;
	imgb.mipLevel = 0;
	imgb.layered = false;
	imgb.texture = outImageTextureb;
	imgb.imageUnit = computePassb.material->m_shader->m_imageBindingPoints["outputImg"];
	imgb.access = TextureAccess::WriteOnly;

	computePassb.images.push_back(imgb);
	m_mainRenderer.addComputePass(computePassb);

	//Compute (Sepia)

	TextureDescription textDescs;
	textDescs.width = 512;
	textDescs.height = 512;
	textDescs.channels = 4;
	textDescs.isHDR = false;

	std::shared_ptr<Texture> outImageTextures = std::make_shared<Texture>(textDescs);

	ShaderDescription computeDescs;
	computeDescs.type = ShaderType::compute;
	computeDescs.computeSrcPath = "./assets/shaders/Lab8/Lab8ComputeSepia.glsl";

	std::shared_ptr<Shader> computeShaders;
	computeShaders = std::make_shared<Shader>(computeDescs);

	std::shared_ptr<Material> computeMats;
	computeMats = std::make_shared<Material>(computeShaders);
	computeMats->setValue("u_image", inputImageTexture);
	computeMats->setValue("u_imageSize", glm::vec2(512, 512));

	ComputePass computePasss;
	computePasss.material = computeMats;
	computePasss.workgroups = { 32, 32, 1 };
	computePasss.barrier = MemoryBarrier::ShaderImageAccess;

	Image imgs;
	imgs.mipLevel = 0;
	imgs.layered = false;
	imgs.texture = outImageTextures;
	imgs.imageUnit = computePasss.material->m_shader->m_imageBindingPoints["outputImg"];
	imgs.access = TextureAccess::WriteOnly;

	computePasss.images.push_back(imgs);
	m_mainRenderer.addComputePass(computePasss);

	// Compute (warp)

	TextureDescription textDescw;
	textDescw.width = 512;
	textDescw.height = 512;
	textDescw.channels = 4;
	textDescw.isHDR = false;

	std::shared_ptr<Texture> outImageTexturew = std::make_shared<Texture>(textDescw);

	ShaderDescription computeDescw;
	computeDescw.type = ShaderType::compute;
	computeDescw.computeSrcPath = "./assets/shaders/Lab8/Lab8ComputeWarp.glsl";

	std::shared_ptr<Shader> computeShaderw;
	computeShaderw = std::make_shared<Shader>(computeDescw);

	std::shared_ptr<Material> computeMatw;
	computeMatw = std::make_shared<Material>(computeShaderw);
	computeMatw->setValue("u_image", inputImageTexture);
	computeMatw->setValue("u_imageSize", glm::vec2(512, 512));
	computeMatw->setValue("u_warp", warp);

	ComputePass computePassw;
	computePassw.material = computeMatw;
	computePassw.workgroups = { 32, 32, 1 };
	computePassw.barrier = MemoryBarrier::ShaderImageAccess;

	Image imgw;
	imgw.mipLevel = 0;
	imgw.layered = false;
	imgw.texture = outImageTexturew;
	imgw.imageUnit = computePassw.material->m_shader->m_imageBindingPoints["outputImg"];
	imgw.access = TextureAccess::WriteOnly;

	computePassw.images.push_back(imgw);
	m_mainRenderer.addComputePass(computePassw);
...
void Lab8::onUpdate(float timestep)
{
	...
	auto warpMat = m_mainRenderer.getComputePass(7).material;
	warpMat->setValue("u_warp", warp);
...
void Lab8::onImGUIRender()
{
	...
	//I have created a tab bar to switch between the compute shader outputs and the main scene.
	...
	if (ImGui::BeginTabBar("Tabs"))
	{
		if (ImGui::BeginTabItem("Lighting"))
		{
			...
			ImGui::ColorEdit3("Light Color", (float*)&m_colour);
		...
		if (ImGui::BeginTabItem("Compute Post Processing"))
		{

			// If I add more compute passes to the GUI, they have to be higher values after my last compute pass.

			GLuint textureID = m_mainRenderer.getComputePass(4).images[0].texture->getID();
			ImGui::Image((void*)(intptr_t)textureID, imageSize, uv0, uv1);

			textureID = m_mainRenderer.getComputePass(5).images[0].texture->getID();
			ImGui::Image((void*)(intptr_t)textureID, imageSize, uv0, uv1);

			textureID = m_mainRenderer.getComputePass(6).images[0].texture->getID();
			ImGui::Image((void*)(intptr_t)textureID, imageSize, uv0, uv1);

			textureID = m_mainRenderer.getComputePass(7).images[0].texture->getID();
			ImGui::Image((void*)(intptr_t)textureID, imageSize, uv0, uv1);
			ImGui::SliderFloat("Warp Factor", (float*)&warp, 0.0f, 50.0f);
