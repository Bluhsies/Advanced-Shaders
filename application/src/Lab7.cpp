...
/*
Lab7 is a combination of lab7 and lab9.
I have done this because I have used the noise texture generated in lab9 as the heightmap for
my floor in this lab.
*/
...
Lab7::Lab7(GLFWWindowImpl& win) : Layer(win)
{
	...
	//Create the compute shader for the noise texture, which is sampled in the floor's tessellation evaluation
	//shader.
	TextureDescription noiseDesc;
	noiseDesc.width = 512;
	noiseDesc.height = 512;
	noiseDesc.channels = 4;
	noiseDesc.isHDR = false;

	std::shared_ptr<Texture> outNoiseTexture = std::make_shared<Texture>(noiseDesc);

	ShaderDescription noiseComputeDesc;
	noiseComputeDesc.type = ShaderType::compute;
	noiseComputeDesc.computeSrcPath = "./assets/shaders/Lab9/noiseTest.glsl";

	std::shared_ptr<Shader> noiseShader;
	noiseShader = std::make_shared<Shader>(noiseComputeDesc);

	//Set uniforms which can be updated in the GUI to change the appearance of the floor.
	std::shared_ptr<Material> noiseComputeMat;
	noiseComputeMat = std::make_shared<Material>(noiseShader);
	noiseComputeMat->setValue("octaves", octaves);
	noiseComputeMat->setValue("frequency", frequency);
	noiseComputeMat->setValue("amplitude", amplitude);
	noiseComputeMat->setValue("persistence", persistence);
	noiseComputeMat->setValue("lacunarity", lacunarity);

	ComputePass noisePass;
	noisePass.material = noiseComputeMat;
	noisePass.workgroups = { 32, 32, 1 };
	noisePass.barrier = MemoryBarrier::ShaderImageAccess;

	Image noiseImg;
	noiseImg.mipLevel = 0;
	noiseImg.layered = false;
	noiseImg.texture = outNoiseTexture;
	noiseImg.imageUnit = noisePass.material->m_shader->m_imageBindingPoints["outputImg"];
	noiseImg.access = TextureAccess::WriteOnly;

	noisePass.images.push_back(noiseImg);
	m_mainRenderer.addComputePass(noisePass);
	...
	//In this iteration, I make use of the TCS and TES shaders, so that I am able to make visible changes
	//to the floor using the noise texture.
	ShaderDescription floorShaderDesc;
	floorShaderDesc.type = ShaderType::tessellationAndGeometry;
	floorShaderDesc.vertexSrcPath = "./assets/shaders/Lab7/Lab7GBVertFloor.glsl";
	...
	floorShaderDesc.geometrySrcPath = "./assets/shaders/Lab7/floorPhongGeo.glsl";
	floorShaderDesc.fragmentSrcPath = "./assets/shaders/Lab7/Lab7GBFragFloor.glsl";
	...
	//Added matrix to reduce floor from floating too much using the noise texture.
	float floorOffset = 6.0f;
	glm::mat4 floorHeightMatrix = glm::mat4(1.0f);
	floorHeightMatrix = glm::translate(floorHeightMatrix, glm::vec3(0.0f, -floorOffset, 0.0f));
	...
	//floorMaterial->setValue("u_tess", spacingValue);
	floorMaterial->setValue("u_heightMap", outNoiseTexture);
	floorMaterial->setValue("u_modelMatrix", floorHeightMatrix);
	floorMaterial->setValue("u_scale", scale);
...
void Lab7::onUpdate(float timestep)
{
	...
	//floorMat->setValue("u_tess", spacingValue);
	floorMat->setValue("u_scale", scale);

	auto bbMat = m_mainScene->m_actors.at(bbIdx).material;
	bbMat->setValue("u_albedo", m_colour);

	auto& noiseComputeMat = m_mainRenderer.getComputePass(0).material;
	noiseComputeMat->setValue("octaves", octaves);
	noiseComputeMat->setValue("amplitude", amplitude);
	noiseComputeMat->setValue("frequency", frequency);
	noiseComputeMat->setValue("persistence", persistence);
	...
	auto& gPass = m_mainRenderer.getRenderPass(2);
	auto& lightPass = m_mainRenderer.getRenderPass(4);
	auto& skyboxPass = m_mainRenderer.getRenderPass(3);
...
void Lab7::onImGUIRender()
{
	...
	//Added tabs to separate lighting and noise shader manipulation
	...
	if (ImGui::BeginTabBar("Tabs"))
	{
		if (ImGui::BeginTabItem("Lighting"))
		{
			...
			GLuint textureID = m_mainRenderer.getRenderPass(1).target->getTarget(0)->getID();

			GLuint textureIDGB = m_mainRenderer.getRenderPass(2).target->getTarget(0)->getID();
			ImGui::Image((void*)(intptr_t)textureIDGB, imageSize, uv0, uv1);

			textureIDGB = m_mainRenderer.getRenderPass(2).target->getTarget(1)->getID();
			ImGui::Image((void*)(intptr_t)textureIDGB, imageSize, uv0, uv1);
			ImGui::SameLine();

			textureIDGB = m_mainRenderer.getRenderPass(2).target->getTarget(2)->getID();
			ImGui::Image((void*)(intptr_t)textureIDGB, imageSize, uv0, uv1);
			ImGui::SameLine();
		...
		if (ImGui::BeginTabItem("Noise"))
		{
			...
			ImGui::SliderFloat("Octaves", (float*)&octaves, 1.0f, 5.0f);
			ImGui::SliderFloat("Amplitude", (float*)&amplitude, 1.0f, 5.0f);
			ImGui::SliderFloat("Frequency", (float*)&frequency, 1.0f, 2.0f);
			ImGui::SliderFloat("Persistence", (float*)&persistence, 0.0f, 1.0f);
			ImGui::SliderFloat("Lacunarity", (float*)&lacunarity, 2.0f, 8.0f);
			ImGui::SliderFloat("Scale", (float*)&scale, 1.0f, 10.0f);

			//ImGui::Image((void*)(intptr_t)textureID, imageSize, uv0, uv1);
			//ImGui::SliderFloat("Floor Spacing", (float*)&spacingValue, 1.0f, 50.f);
