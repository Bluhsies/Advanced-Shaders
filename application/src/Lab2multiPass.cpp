...
Lab2multiPass::Lab2multiPass(GLFWWindowImpl& win) : Layer(win)
{
	...
	**************************/
	...
	//Create the actor and add it to m_actors
	Actor invertScreen;
	...
	//Add the renderpass to the scene and reset the scene.
	...
	//Add the renderpass and reset the scene.
	...
	//Lens flare shader
	// This was an attempt at a lens flare shader, but it is more like a god-ray shader
	...
	ShaderDescription thermalShaderDesc;
	thermalShaderDesc.type = ShaderType::rasterization;
	thermalShaderDesc.vertexSrcPath = "./assets/shaders/Lab2/Lab2ThermalVert.glsl";
	thermalShaderDesc.fragmentSrcPath = "./assets/shaders/Lab2/Lab2ThermalFrag.glsl";

	std::shared_ptr<Shader> thermalShader;
	thermalShader = std::make_shared<Shader>(thermalShaderDesc);

	VBOLayout thermalLayout = {
	...
	std::shared_ptr<VAO> thermalVAO;
	thermalVAO = std::make_shared<VAO>(screenIndices);
	thermalVAO->addVertexBuffer(screenVertices, thermalLayout);

	std::shared_ptr<Material> thermalMaterial;
	thermalMaterial = std::make_shared<Material>(thermalShader);
	thermalMaterial->setValue("u_originalTexture", mainPass.target->getTarget(0));

	Actor thermalScreen;
	thermalScreen.geometry = thermalVAO;
	thermalScreen.material = thermalMaterial;
	thermalIdx = m_screenScene->m_actors.size();
	m_screenScene->m_actors.push_back(thermalScreen);

	RenderPass thermalPass;
	thermalPass.scene = m_screenScene;
	thermalPass.parseScene();
	thermalPass.target = std::make_shared<FBO>(m_winRef.getSize(), thermalFBOLayout);
	thermalPass.viewPort = { 0,0,m_winRef.getWidth(), m_winRef.getHeight() };
	thermalPass.camera.projection = glm::ortho(0.f, width, height, 0.f);
	thermalPass.UBOmanager.setCachedValue("b_camera2D", "u_view", thermalPass.camera.view);
	thermalPass.UBOmanager.setCachedValue("b_camera2D", "u_projection", thermalPass.camera.projection);

	m_mainRenderer.addRenderPass(thermalPass);
	m_screenScene.reset(new Scene);
	...
	finalMaterial->setValue("u_originalTexture", thermalPass.target->getTarget(0));
...
void Lab2multiPass::onRender() const
{
	m_mainRenderer.render();
}
void Lab2multiPass::onUpdate(float timestep)
{
...
void Lab2multiPass::onImGUIRender()
{
	...
	//Output each of the post-processing effects to a different window within the GUI.
	...
	if (ImGui::InputFloat("Edge Detection Threshold", (float*)&deThreshold))
	{
		...
		auto& edgeMaterial = m_mainRenderer.getRenderPass(5).scene->m_actors.at(edgeIdx).material;
		edgeMaterial->setValue("threshold", deThreshold);
		//Allows you to change the edge detection threshold for the shader, to produce a different output.
	}
...
void Lab2multiPass::onKeyPressed(KeyPressedEvent& e)
{
	for (auto it = m_mainScene->m_actors.begin(); it != m_mainScene->m_actors.end(); ++it)
	{
		it->onKeyPress(e);
