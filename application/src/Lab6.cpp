...
Lab6::Lab6(GLFWWindowImpl& win) : Layer(win)
{
	...
	//The floor now makes use of a geometry shader to take in the values for the
	//vertex normal, texture coordinates and fragment position.
	...
	//Billboards
	//Set up bill boards (trees) in the scene to be rendered. Make use of a geometry shader, which
	//takes data passed to the vertex shader and outputs it to the fragment shader.
...
void Lab6::onRender() const
{
	m_mainRenderer.render();
}

void Lab6::onUpdate(float timestep)
{
...
void Lab6::onKeyPressed(KeyPressedEvent& e)
{
	for (auto it = m_mainScene->m_actors.begin(); it != m_mainScene->m_actors.end(); ++it)
	{
		it->onKeyPress(e);
